...
5. Install `beads-mcp` with `uv tool install beads-mcp`
6. Configure MCP for GitHub Copilot in user-level `mcp.json` (global for all projects)
7. Configure VS Code bash-first terminals (Git Bash (.venv) on Windows, bash on Linux/macOS)
8. Initialize Beads if not already initialized
9. Install managed git hooks (guard + Beads chaining)
10. Run quality verification
11. Commit setup-generated tracked files when present (`.gitignore`, `.beads/hooks/*`)
12. Require session kickoff prompt before any implementation work
...
4. Verify toolchain:
   ...
   dolt version
   beads-mcp --help
5. Ensure user-level MCP config includes Beads server:
   - User-level (all projects): `%APPDATA%\Code\User\mcp.json` on Windows,
     `~/Library/Application Support/Code/User/mcp.json` on macOS,
     `~/.config/Code/User/mcp.json` on Linux
   - Merge only: preserve existing MCP servers and avoid deleting unrelated entries
6. Commit setup-generated tracked files if they exist:
...
8. If Windows PATH entries were added during setup, remind user to restart VS Code and open a new terminal.
9. Before any implementation, run `.github/prompts/start-work-session.prompt.md`.
