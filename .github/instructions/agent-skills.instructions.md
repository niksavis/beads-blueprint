...
description: "Use when creating, reviewing, or debugging SKILL.md files and skill metadata (name, description, frontmatter, discovery triggers) for GitHub Copilot."
---
...
Run the helper script at `<SKILL_SCRIPT_PATH>` to automate common tasks.

See `<SKILL_REFERENCE_PATH>` for detailed documentation.

Use `<SKILL_TEMPLATE_PATH>` as a starting point.
```
...
- [ ] Step 1: Configure environment - see `<SETUP_REFERENCE_SECTION>`
- [ ] Step 2: Build project - see `<BUILD_REFERENCE_SECTION>`
- [ ] Step 3: Deploy to staging - see `<STAGING_REFERENCE_SECTION>`
- [ ] Step 4: Run validation - see `<VALIDATION_REFERENCE_SECTION>`
- [ ] Step 5: Deploy to production - see `<PRODUCTION_REFERENCE_SECTION>`
```
